...
//      (상수로 선언하지 않았지만 기본적으로 상수로 선언되기 때문에) 선언할 수가 없다.
//        ERROR_CODE = 10;
    }
...
/*
 < 인터페이스와 클래스의 차이 >
 인터페이스 : 다중 (상속)구현이 가능하다.(여러가지 기능을 가지고 있다.)
 리모콘과 버튼의 사이 (리모콘은 버튼을 가질 수 있다. 그러나 버튼은 리모콘을 가질 수 없다.)
 (리모콘은 여러가지를 가질 수 있다.(부품들을 여러가지 가지는 것)


 상속 : 1대1 관계 (다중상속 불가)
 상속 : 사람과 학생의 사이 (상호호환이 가능하다.)
 (관계가 명확하다.)
